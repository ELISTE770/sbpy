...
# ======================================================================
SYSTEM_DIAGNOSE = (
    "אתה מומחה בכיר לאבחון ופתרון שגיאות Python וסביבות פיתוח. המטרה שלך היא לבצע אבחון מעמיק, למצוא את הסיבה השורשית המדויקת ולספק תיקון קונקרטי.\n"
    "עליך לפעול לפי הכללים הבאים:\n"
    "1. זהה תמיד את שורש הבעיה באופן מדויק והסבר אותו בבהירות בשדה 'cause'.\n"
    "2. בדיקת שפות ומקלדות זרות (Multi-language & Keyboard Layout Detection):\n"
    "   אם הקוד, השגיאה או הפקודה מכילים תווים שאינם באנגלית (עברית, ערבית, רוסית/קירילית, יוונית וכו'):\n"
    "   א. בדוק מיד האם מדובר בהקלדה שגויה של פריסת מקלדת (Keyboard Layout Mismatch / QWERTY transliteration) – למשל הקלדה בעברית/ערבית/רוסית של פקודות shell (כגון pip install, git commit, python main.py) או שורות קוד פייתון שלמות.\n"
    "   ב. בדוק האם מדובר בפסאודו-קוד או מילות מפתח מתורגמות (כגון 'הדפס', 'אם', 'החזר', 'הגדר', 'נכון', 'שקר').\n"
    "   ג. בכל מקרה של שפה זרה או הקלדת מקלדת שגויה, שחזר ותרגם את כל השורה או הפקודה לקוד פייתון או פקודת shell תקינה לחלוטין באנגלית והחזר אותה במדויק בשדה 'patch'.\n"
    "3. זיהוי פקודות טרמינל / Shell שנכתבו ב-REPL:\n"
    "   אם המשתמש ניסה להריץ פקודת מערכת (כמו pip install, cd, ls, dir, node, npm, git) בתוך ה-REPL של פייתון, ציין זאת בבירור והחזר ב-'patch' את הפקודה התקינה.\n"
    "4. איכות הקוד בשדה 'patch':\n"
    "   השדה 'patch' חייב להכיל אך ורק קוד/פקודה תקינה, מדויקת ומוכנה להרצה (ללא markdown וללא הערות מיותרות).\n"
    "5. החזר JSON מדויק לפי הסכמה בלבד."
)
...
def diagnose_prompt(
    *,
    ...
    if code:
        parts += ["", "הקוד סביב השגיאה / שורת הקלט שנכשלה:", "```python", code, "```"]
    if traceback_tail:
        parts += ["", "סוף ה-traceback:", "```", traceback_tail, "```"]
    if local_notes:
        parts += ["", f"מה שהבדיקה המקומית כבר מצאה: {local_notes}"]

    # הנחיות עומק ייעודיות לזיהוי שפות, מקלדות ופקודות
    parts += [
        "",
        "💡 הנחיות לאבחון מעמיק:",
        "1. נתח האם הטקסט מכיל תווים שאינם אנגליים הנובעים מאי-התאמת פריסת מקלדת (עברית, ערבית, רוסית וכו') ותרגם אותו במדויק לקוד אנגלי.",
        "2. נתח האם המשתמש התכוון לפקודת Shell, תחביר ספציפי, ביטוי לוגי מורכב או ייבוא חסר.",
        "3. בשדה 'patch' החזר את שורת הקוד / הפקודה המתוקנת במלואה מוכנה להחלה.",
        "",
        "מה הסיבה השורשית המדויקת ומה התיקון המומלץ?",
    ]
    return "\n".join(parts)
